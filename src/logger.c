...
enum {
	/* Logger type */
	kConsoleLogger = 1 << 0, kFileLogger = 1 << 1,
...
static int is_set_flag(int flag) {
	int pika = _log_flag & flag;
	return pika;
}
...
static long vflog(FILE *fp, char levelc, const char *timestamp, long threadID,
	...
	/* decorate print */
	char decore[500];
	if (is_set_flag(LogDecorate_LogLevel)) {
		size = sprintf(decore, "%c ", levelc);
		if (size)
			totalsize += size;
	}
	if (is_set_flag(LogDecorate_Date)) {
		size = sprintf(decore + totalsize, "%.*s ", 8, timestamp);
		if (size)
			totalsize += size;
	}
	if (is_set_flag(LogDecorate_Time)) {
		size = sprintf(decore + totalsize, "%s ", timestamp + 9);
		if (size)
			totalsize += size;
	}
	if (is_set_flag(LogDecorate_ThreadId)) {
		size = sprintf(decore + totalsize, "%ld ", threadID);
		if (size)
			totalsize += size;
	}
	if (is_set_flag(LogDecorate_FileName)) {
		size = sprintf(decore + totalsize, "%s:%d", file, line);
		if (size)
			totalsize += size;
	}

	if (totalsize > 0) {
		size = fprintf(fp, "%s: ", decore);
		if (size)
			totalsize += 2;
	}
	/*end decorate print */
...
void logger_log(LogLevel level, const char *file, int line, const char *fmt,
	...
	if (is_set_flag(LogDecorate_Time) || is_set_flag(LogDecorate_Date)) {
	...
	if (hasFlag(s_logger, kFileLogger)) {
		if (rotateLogFiles()) {
			va_start(farg, fmt);
			s_flog.currentFileSize += vflog(s_flog.output, levelc, timestamp,
					threadID, file, line, fmt, farg, currentTime,
					&s_flog.flushedTime);
			va_end(farg);
		}
	}
